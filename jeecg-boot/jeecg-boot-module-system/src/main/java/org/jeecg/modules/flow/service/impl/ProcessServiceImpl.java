    ...
    public void complete(ActivitiBaseEntity activitiBaseEntity, String module) {
        ...
        for(Map.Entry<String, Object> entry : activitiBaseEntity.getProcessParams().entrySet()){
            ...
            if (StringUtils.isNotEmpty(comment)) {
                identityService.setAuthenticatedUserId(FlowSecurityUtils.getUsername());
            ...
            taskService.claim(activitiBaseEntity.getTaskId(), FlowSecurityUtils.getUsername());
            ...
            for (BizTodoItem update: updateList) {
                ...
                if (update.getTodoUserId().equals(FlowSecurityUtils.getUsername())) {
                    update.setIsView("1");
                    update.setIsHandle("1");
                    update.setHandleUserId(FlowSecurityUtils.getUsername());
                    update.setHandleUserName(FlowSecurityUtils.getNickName());
...
//                .listPage((pageNum - 1) * pageSize, pageNum * pageSize);
        list.forEach(instance -> {
            ...
            SysUser sysUser=sysUserService.getUserByName(instance.getAssignee());
            if (sysUser != null) {
